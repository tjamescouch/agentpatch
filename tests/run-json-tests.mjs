...
function testJsonPartialFailure() {
  const cwd = mkTmp();
  // add would succeed, but update fails (file doesn't exist) — atomic: neither should write
  const patch = [
    '*** Begin Patch',
    '*** Add File: good.txt',
    '+good',
  ...
  const r = apply(patch, cwd);
  assert(r.code === 1, `exit code should be 1 for validation failure, got ${r.code}`);
  const j = parseJson(r.out);
  assert(j.success === false, 'success should be false');
  // atomic: good.txt must NOT have been written
  assert(!j.applied.includes('good.txt'), 'atomic: good.txt should NOT be applied when another op fails');
